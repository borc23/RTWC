...
def validate_dataset(data_dir, splits=["train", "val"]):
    ...
    stats = {"valid": True, "errors": [], "warnings": [], "splits": {}}
    ...
    for split in splits:
        ...
        if labels_without_images:
            stats["warnings"].append(
                f"{split}: {len(labels_without_images)} labels without corresponding images"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare and validate dataset for YOLO training."
    )
    parser.add_argument(
        "--data-dir", type=str, required=True, help="Path to dataset directory."
    )
    parser.add_argument(
        "--output-yaml", type=str, default=None, help="Output path for data.yaml."
    )
    parser.add_argument(
        "--stats-output",
        type=str,
        default="outputs/data_stats.json",
        help="Output path for statistics.",
    )
    parser.add_argument(
        "--classes", type=str, nargs="+", default=None, help="Class names (in order)."
    )
    ...
    for split, split_stats in stats["splits"].items():
        ...
        if split_stats.get("image_size_stats"):
            ...
            print(
                f"  Image sizes: {size_stats['min_width']}x{size_stats['min_height']} to {size_stats['max_width']}x{size_stats['max_height']}"
            )
    ...
    config = create_data_yaml(
        data_dir, output_yaml, class_names=class_names, test_exists="test" in splits
    )
